...
This is the same setup whether you're inside this consumer repo's workspace or a true standalone clone — the only difference is that inside the workspace, `pnpm`'s `link-workspace-packages=true` (root `.npmrc`) resolves `@hrtbrxkxr/shared-ui`/`@hrtbrxkxr/shared-utils` to the local `packages/*` source instead of fetching from GitHub Packages, so edits there show up immediately without needing a publish.
...
Until you've actually pushed a tag at least once, standalone clones of `module-a` (or any other module) will get a `404` trying to install `@hrtbrxkxr/shared-ui`/`@hrtbrxkxr/shared-utils` from GitHub Packages — there's nothing published yet.
...
- **`pnpm install` 404s on `@hrtbrxkxr/shared-ui` in a standalone clone**: either `GITHUB_TOKEN` isn't exported (needs `read:packages` scope), or no version has been published yet — see "Publishing shared-ui / shared-utils" above.
